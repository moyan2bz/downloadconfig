...
(function() {
    ...
    const DOMAIN_CONFIG = {
        ...
        API_BASE_DEFAULT_ENCRYPTED: "OHFVJHpMO3RAMH1fL2NHPyNDGSBBQn4WBTpcGz1AFjRlBF9nA0thFw4rSVR1BU5qOHFVJHpMO3RAMH1fL2NHPw==",
        ENCRYPT_KEY: "K7mP2xQ9vR3nL5wZ",
        ENCRYPT_SALT: "sF8tH4jM6bN1cV0e",
        IGNORED_VERSION_KEY: "auto_dispatch_ignored_version",
        UPDATE_SCRIPT_URL: "https://gitee.com/moyan1bz/downloadconfig/raw/master/javascript.user.js"
    };
                        ...
                        if (response.status === 200) {
                            ...
                            if (config && config.version) {
                                ...
                                if (remoteVersion > VERSION) {
                                    fetchCommitChangelog((changelog) => {
                                        const updateInfo = {
                                            version: remoteVersion,
                                            changelog: changelog || '暂无更新日志'
                                        };
                                        resolve(updateInfo);
                                    });
                                    return;
    ...
    function fetchCommitChangelog(callback) {
        GM_xmlhttpRequest({
            method: "GET",
            url: "https://gitee.com/api/v5/repos/moyan1bz/downloadconfig/commits?sha=master&per_page=1",
            timeout: 4000,
            onload: function(response) {
                try {
                    if (response.status === 200) {
                        ...
                        if (Array.isArray(commits) && commits.length > 0) {
                            ...
                            if (commit.commit && commit.commit.message) {
                                ...
                                const body = lines.slice(1).filter(line => 
                                    line.trim() && !line.includes('Signed-off-by:')
                                ).join('\n');
                                callback(body.trim() || '暂无更新日志');
                                return;
                            }
                        }
                    }
    ...
    function showUpdateDialog(updateInfo) {
        ...
        dialog.innerHTML = `
            <div style="background: #28a745; color: white; padding: 12px; border-radius: 8px 8px 0 0; text-align: center; font-weight: bold; margin: -31px -30px 20px -30px;">更新提示</div>
            <div style="text-align: center; margin-bottom: 20px;">
                <div style="font-size: 30px; margin-bottom: 15px;">🔔</div>
                <div style="font-size: 18px; font-weight: bold; color: #333;">检测到新版本 - ${updateInfo.version}</div>
                <div style="font-size: 14px; color: #666; margin-top: 5px;">当前版本 - ${VERSION}</div>
            </div>
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; margin-bottom: 15px; max-height: 150px; overflow-y: auto; border-left: 4px solid #28a745;">
                <div style="font-size: 12px; color: #666; font-weight: bold; margin-bottom: 8px;">更新日志</div>
                <div style="font-size: 12px; color: #555; line-height: 1.6; white-space: pre-wrap;">${updateInfo.changelog}</div>
            </div>
            <div style="background: #fff3cd; border-radius: 8px; padding: 12px; margin-bottom: 20px; font-size: 12px; color: #856404; line-height: 1.5;">
                <strong>💡 升级提示：</strong>文件下载完成自动跳转升级页，点击 overwrite（更新），脚本自动安装。
            </div>
            <div style="display: flex; gap: 10px; justify-content: center;">
                <button id="update-later-btn" style="padding: 10px 20px; background: transparent; color: #666; border: 1px solid #ddd; border-radius: 8px; cursor: pointer; font-size: 14px;">下次再说</button>
                <button id="update-ignore-btn" style="padding: 10px 20px; background: transparent; color: #666; border: 1px solid #ddd; border-radius: 8px; cursor: pointer; font-size: 14px;">忽略本次更新</button>
                <button id="update-now-btn" style="padding: 10px 30px; background: #28a745; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: bold; font-size: 14px;">立即更新</button>
            </div>
        `;
    ...
    async function initDomainConfig() {
        ...
        if (!apiBaseUrl) {
            apiBaseUrl = decryptApiUrl(DOMAIN_CONFIG.API_BASE_DEFAULT_ENCRYPTED);
        }
